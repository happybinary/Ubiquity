...
- (id) init {
    ...
    if (self) {
        ...
        self.tabBarItem = [[UITabBarItem alloc] initWithTitle:nil image:nil tag:3];
        
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7) {
            [self.tabBarItem setImage:[[UIImage imageNamed:@"ic_home"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
            [self.tabBarItem setSelectedImage:[[UIImage imageNamed:@"tab_home_focus"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
            
        }
        else
            [[self tabBarItem] setFinishedSelectedImage:[UIImage imageNamed:@"tab_home_focus"] withFinishedUnselectedImage:[UIImage imageNamed:@"ic_home"]];
        
        
        AppGlobalData *sharedManager = [AppGlobalData sharedManager];
        CGFloat paddingUnit = 6;
        if (sharedManager.Include_iPad_Interface && UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        {
            paddingUnit = 7;
        }
        self.tabBarItem.imageInsets = UIEdgeInsetsMake(paddingUnit, 0, -paddingUnit, 0);
    }
    return self;
}
...
- (void)viewWillLayoutSubviews {
    ...
    if([self respondsToSelector:@selector(topLayoutGuide)]) {
        CGFloat topY = [self.topLayoutGuide length];;
        ...
        UIEdgeInsets insets = UIEdgeInsetsMake(topY, 0, 0, 0);
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if ([touch.view isKindOfClass:[IPRotaryWheelPicker class]] ||
        [touch.view.superview isKindOfClass:[IPRotaryWheelPicker class]])
    {
        return NO;
    }
