...
@implementation IPKeyboardControls
...
- (id)initWithFields:(NSArray *)fields
{
    if (self = [super initWithFrame:CGRectMake(0.0f, 0.0f, 320.0f, 44.0f)])
    {
        ...
        [self setSegmentedControl:[[UISegmentedControl alloc] initWithItems:@[ NSLocalizedStringFromTable(@"Previous", @"IPKeyboardControls", @"Previous button title."),
                                                                               NSLocalizedStringFromTable(@"Next", @"IPKeyboardControls", @"Next button title.") ]]];
        [self.segmentedControl addTarget:self action:@selector(segmentedControlValueChanged:) forControlEvents:UIControlEventValueChanged];
        [self.segmentedControl setMomentary:YES];
        [self.segmentedControl setSegmentedControlStyle:UISegmentedControlStyleBar];
        [self.segmentedControl setEnabled:NO forSegmentAtIndex:IPKeyboardControlsDirectionPrevious];
        [self.segmentedControl setEnabled:NO forSegmentAtIndex:IPKeyboardControlsDirectionNext];
        [self setSegmentedControlItem:[[UIBarButtonItem alloc] initWithCustomView:self.segmentedControl]];
        
        //[self setSegmentedControlTintControl:[UIColor redColor]];
        [self setBarTintColor:[UIColor blueColor]];
        
        [self setDoneButton:[[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Done", @"IPKeyboardControls", @"Done button title.")
                                                             style:UIBarButtonItemStyleDone
                                                            target:self
                                                            action:@selector(doneButtonPressed:)]];
        
        [self setVisibleControls:(IPKeyboardControlPreviousNext | IPKeyboardControlDone)];
...
-(void)setTranslucent:(BOOL)translucent
{
    ...
    {
        if ([self.toolbar respondsToSelector:@selector(setTranslucent:)]) {
...
- (void)setPreviousTitle:(NSString *)previousTitle
{
    ...
    {
        [self.segmentedControl setTitle:previousTitle forSegmentAtIndex:IPKeyboardControlsDirectionPrevious];
...
- (void)setNextTitle:(NSString *)nextTitle
{
    ...
    {
        [self.segmentedControl setTitle:nextTitle forSegmentAtIndex:IPKeyboardControlsDirectionNext];
...
- (void)setVisibleControls:(IPKeyboardControl)visibleControls
{
...
- (void)segmentedControlValueChanged:(id)sender
{
    ...
    {
        case IPKeyboardControlsDirectionPrevious:
            [self selectPreviousField];
            break;
        case IPKeyboardControlsDirectionNext:
...
- (void)updateSegmentedControlEnabledStates
{
    ...
    if (index != NSNotFound)
    {
        [self.segmentedControl setEnabled:(index > 0) forSegmentAtIndex:IPKeyboardControlsDirectionPrevious];
        [self.segmentedControl setEnabled:(index < [self.fields count] - 1) forSegmentAtIndex:IPKeyboardControlsDirectionNext];
    }
}
...
- (void)selectPreviousField
{
    ...
    if (index > 0)
    {
        ...
        {
            [self.delegate keyboardControls:self selectedField:field inDirection:IPKeyboardControlsDirectionPrevious];
        }
    }
}
...
- (void)selectNextField
{
    ...
    if (index < [self.fields count] - 1)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(keyboardControls:selectedField:inDirection:)])
        {
            [self.delegate keyboardControls:self selectedField:field inDirection:IPKeyboardControlsDirectionNext];
        }
    }
}
...
- (NSArray *)toolbarItems
{
    ...
    if (self.visibleControls & IPKeyboardControlPreviousNext)
    {
        [items addObject:self.segmentedControlItem];
    }
    
    if (self.visibleControls & IPKeyboardControlDone)
    {
